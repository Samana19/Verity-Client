...
const Product = ({ item, provider, account, verity, togglePop }) => {
  const [order, setOrder] = useState(null);
  const [hasBought, setHasBought] = useState(null);

  const fetchDetails = async () => {
    const events = await verity.queryFilter("Buy");
    ...
    const order = await verity.orders(account, orders[0].args.orderId);
    setOrder(order);
  };

  const buyHandler = async () => {
    const signer = await provider.getSigner();
    let transaction = verity
      .connect(signer)
    ...
    <div className="product">
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          <p>
            <small>Ships From </small> Verity
          </p>
          <p>
            <small>Sold By</small> Verity
          </p>

          {order && (
            <div className="product__bought">
              Item bought on <br></br>
              <strong>
                {new Date(
